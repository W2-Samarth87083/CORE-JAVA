...
Question:1. Create a new eclipse project day4_assignments , Create a class Point2D , 
in package - "com.app.geometry"   : for representing a point in x-y co-ordinate 
system. 
a) Create a parameterized constructor to init x & y co-ords. 
b) Add a method to return string form of  point's x & y co-ords 
Hint :  public String getDetails())  
c) Add isEqual method to Point2D class :a boolean returning method : must 
return true if n only if both points are having same x,y co-ords or false 
otherwise. 
d) Add calculateDistance method to calculate distance between current point 
and specified point & return the distance to the caller. 
Hint : Use distance formula . Use java.lang.Math class methods --sqrt, pow 
etc. 
e) Write TestPoint class , in package "tester" , with a main method Accept co 
...
f) Invoke isEqual & display if points are same or different (i.e p1 & p2 are 
